...
if [[ "$CONTAINER_NAME" == "gnb" ]]; then
    sed -i "s/NETWORK_MCC/$HMCC/g" gnb.yml
    sed -i "s/NETWORK_MNC/$HMNC/g" gnb.yml
...
if [[ "$CONTAINER_NAME" == "cu" ]]; then
    sed -i "s/NETWORK_MCC/$HMCC/g" cu.yml
    sed -i "s/NETWORK_MNC/$HMNC/g" cu.yml
...
if [[ "$CONTAINER_NAME" == "du_1" ]]; then
    sed -i "s/NETWORK_MCC/$HMCC/g" du_b200.yml
    sed -i "s/NETWORK_MNC/$HMNC/g" du_b200.yml
...
if [[ "$CONTAINER_NAME" == "du_2" ]]; then
    sed -i "s/NETWORK_MCC/$HMCC/g" du_b205.yml
    sed -i "s/NETWORK_MNC/$HMNC/g" du_b205.yml
